...
#define CP__SST_FIXED_POINT 6 //previously 16
...
control_process_spinup_cb(struct control_process *cps)
{
	...
	if (cp_spinning_ready() &&
		(spinup_process.step < CP__SST_SAFE_FOR_CLOSED_LOOP) &&
		(spinup_process.hold < 1)) {
...
//			int powerv = 1000/(spinup_process.step >> CP__SST_FIXED_POINT);
//			power = (135.0/2567)*powr(2,powerv)-(-71953/29);


	/* Give up if the commutation speed exeeds limit and the spinning
	 * process is still not happy
	 */
	if ((spinup_process.step <= CP__SST_MIN_STEP)) {
		spinup_process.step = (u32)(CP__SST_MIN_STEP);
